...
# Get the date for five years ago
five_years_ago = (datetime.today() - timedelta(days=5*365)).strftime('%Y-%m-%d')

# Fetch historical data for the past 5 years once
historical_data_df = ticker.history(start=five_years_ago, end=today, interval='1d')
...
def fetch_and_plot(i):
    ...
    # Plot historical data (past 5 years) with area under the curve filled in green
    ax.fill_between(historical_data_df.index, historical_data_df['Close'], color='green', alpha=0.5, label='Historical (5 Years)')
