...
three_months_ago = (datetime.today() - timedelta(days=92)).strftime('%Y-%m-%d')

# Fetch historical data for the past three months once
historical_data_df = ticker.history(start=three_months_ago, end=today, interval='1d')
...
def fetch_and_plot(i):
    global previous_close  # Use global variable to track the previous closing price
    ...
    # Plot historical data (past three months) with area under the curve filled in green
    ax.fill_between(historical_data_df.index, historical_data_df['Close'], color='green', alpha=0.5, label='Historical (3 Months)')
