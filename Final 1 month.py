...
# Fetch historical data for the past month once
historical_data_df = ticker.history(start=one_month_ago, end=today, interval='1d')
...
def fetch_and_plot(i):
    ...
    # Plot historical data (past month) with area under the curve filled in green
    ax.fill_between(historical_data_df.index, historical_data_df['Close'], color='green', alpha=0.5, label='Historical (1 Month)')
